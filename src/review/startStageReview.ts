...
function syncRuntimeIssues(runtime: RuntimeApp): void {
  if (runtime instanceof ViewerApp) {
    runtime.updateIssues();
  }
}

export async function startStageReview(mode: StageReviewMode, sceneId: string): Promise<void> {
  ...
  if (!sceneContainer || !shellRoot) {
    throw new Error("Missing review shell containers.");
  }
  ...
  const runtime: RuntimeApp =
    mode === "stage4-xr" || mode === "stage5-xr"
      ? await XrReviewApp.create(sceneContainer, bundle)
      : await ViewerApp.create(sceneContainer, bundle);
